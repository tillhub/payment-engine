...
class TerminalDisconnectContract : ActivityResultContract<Terminal, TerminalOperationStatus>() {
    override fun createIntent(context: Context, input: Terminal): Intent {
        return when (input) {
            ...
            else -> throw UnsupportedOperationException("Disonnect only supported for S-POS terminals")
        }
    }
    ...
    companion object {
        private const val CONNECT_ACTION = "de.spayment.akzeptanz.S_SWITCH_DISCONNECT"
